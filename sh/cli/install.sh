...
#
# Usage:
#   curl -fsSL --proto '=https' https://raw.githubusercontent.com/neosantara-xyz/jelma/neosantara/sh/cli/install.sh | bash
#
...
SPAWN_BRANCH="${SPAWN_BRANCH:-neosantara}"
SPAWN_CDN="https://raw.githubusercontent.com/${SPAWN_REPO}/${SPAWN_BRANCH}/sh"
...
log_info()  { printf '%b[jelma]%b %s\n' "$GREEN" "$NC" "$1"; }
log_step()  { printf '%b[jelma]%b %s\n' "$CYAN" "$NC" "$1"; }
log_warn()  { printf '%b[jelma]%b %s\n' "$YELLOW" "$NC" "$1"; }
log_error() { printf '%b[jelma]%b %s\n' "$RED" "$NC" "$1"; }
...
# --- Helper: ensure jelma works immediately and in future sessions ---
...
ensure_in_path() {
    ...
    # 2. If jelma not in PATH, symlink into /usr/local/bin for immediate availability
    #    Try in order: direct write → passwordless sudo → prompt for password
    #    Also symlink bun so that jelma's #!/usr/bin/env bun shebang resolves
    local linked=false
    local bun_path
    ...
    if [ "$jelma_in_path" = false ]; then
        if [ -d /usr/local/bin ] && [ -w /usr/local/bin ]; then
            safe_ln_sf "${install_dir}/jelma" /usr/local/bin/jelma && linked=true
            ...
            safe_ln_sf "${install_dir}/jelma" /usr/local/bin/jelma sudo 2>/dev/null && linked=true
            if [ -n "$bun_path" ] && [ ! -x /usr/local/bin/bun ]; then
                safe_ln_sf "$bun_path" /usr/local/bin/bun sudo 2>/dev/null || true
            fi
        elif command -v sudo &>/dev/null; then
            # Last resort: ask for password
            log_step "Adding jelma to /usr/local/bin (may require your password)..."
            safe_ln_sf "${install_dir}/jelma" /usr/local/bin/jelma sudo && linked=true || true
    ...
    #    for future sessions. ~/.bun/bin is required by jelma's #!/usr/bin/env bun shebang.
    local rc_file=""
    ...
    local marker_start="# >>> jelma >>>"
    local marker_end="# <<< jelma <<<"
    ...
    echo ""
    SPAWN_NO_UPDATE_CHECK=1 PATH="${install_dir}:${PATH}" "${install_dir}/jelma" version
    echo ""
    local all_ready=true
    if [ "$jelma_in_path" = false ] && [ "$linked" = false ]; then
        all_ready=false
    fi
    ...
    if [ "$all_ready" = true ]; then
        printf '%b[jelma]%b Run %bjelma%b to get started\n' "$GREEN" "$NC" "$BOLD" "$NC"
    else
        printf '%b[jelma]%b To start using jelma, run:\n' "$GREEN" "$NC"
        echo ""
        echo "    exec \$SHELL"
        echo ""
    fi
...
build_and_install() {
    ...
    mkdir -p "${INSTALL_DIR}"
    cp "${tmpdir}/cli.js" "${INSTALL_DIR}/jelma"
    chmod +x "${INSTALL_DIR}/jelma"

    log_info "Installed jelma to ${INSTALL_DIR}/jelma"
...
log_step "Installing jelma via bun..."
build_and_install
