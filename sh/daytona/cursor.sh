...
if [[ -n "${SPAWN_CLI_DIR:-}" && -f "$SPAWN_CLI_DIR/packages/cli/src/daytona/main.ts" ]]; then
    exec bun run "$SPAWN_CLI_DIR/packages/cli/src/daytona/main.ts" cursor "$@"
fi

# Remote — download bundled daytona.js from GitHub release
DAYTONA_JS=$(mktemp)
trap 'rm -f "$DAYTONA_JS"' EXIT
curl -fsSL --proto '=https' "https://github.com/neosantara-xyz/jelma/releases/download/daytona-latest/daytona.js" -o "$DAYTONA_JS" \
