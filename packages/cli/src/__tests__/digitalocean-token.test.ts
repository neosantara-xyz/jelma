...
describe("doApi 401 OAuth recovery", () => {
  ...
  it("attempts OAuth recovery on 401 before throwing", async () => {
    ...
    // Verify recovery was attempted: API call + connectivity check
    const apiCalls = urls.filter((u) => u.includes("api.digitalocean.com"));
    const oauthCalls = urls.filter((u) => u.includes("cloud.digitalocean.com"));
